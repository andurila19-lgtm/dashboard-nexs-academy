...
import { DoorOpen, Plus, Search, Edit2, Trash2, Users } from 'lucide-react';
import { Modal, ConfirmDialog } from '@/components/ui/Modal';
import { StatusAktifBadge } from '@/components/ui/Badge';
import { useNEXSStore } from '@/lib/store';
import { Ruangan, StatusAktif } from '@/lib/types';
import { useToast } from '@/components/ui/Toast';

export default function MasterRuanganPage() {
  const { ruangan, addRuangan, updateRuangan, deleteRuangan } = useNEXSStore();
  ...
  const handleSave = (e: React.FormEvent) => {
    e.preventDefault();
    if (!nama) {
      toast.warning('Form Belum Lengkap', 'Nama ruangan harus diisi.');
      return;
    }

    if (editingItem) {
      updateRuangan(editingItem.id, { nama, kapasitas, status });
      toast.success('Ruangan Diperbarui', `Informasi ${nama} berhasil disimpan.`);
    } else {
      addRuangan({ nama, kapasitas, status });
      toast.success('Ruangan Ditambahkan', `${nama} berhasil didaftarkan.`);
    }
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500 mt-0.5">
            Daftar ruangan kelas dan kapasitas yang tersedia di NEXS.
          </p>
        </div>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-slate-50 border-b border-slate-200/80 text-xs font-semibold text-slate-500 uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-3.5">Kapasitas Maksimal</th>
      ...
      <Modal
        ...
        title={editingItem ? 'Edit Data Ruangan' : 'Tambah Ruangan Baru'}
        maxWidth="md"
      >
        <form onSubmit={handleSave} className="space-y-4">
          <div>
            ...
            <input
              type="text"
              placeholder="Contoh: Room 1 (Tokyo)"
              value={nama}
      ...
      <ConfirmDialog
        ...
        onConfirm={() => {
          if (deleteTargetId) {
            deleteRuangan(deleteTargetId);
            setDeleteTargetId(null);
            toast.info('Ruangan Dihapus', 'Data ruangan telah dihapus.');
          }
        }}
        title="Hapus Data Ruangan?"
        message="Ruangan ini akan dihapus dari sistem."
